...
import {UploadNewArticle} from "../../http/contentAPI";
...
const NewArticle = () => {
    ...
    const uploadArticle = () => {
        ...
        UploadNewArticle(formData).then(() => {
    ...
    return (
        <div className="fullscreen-frame">
            <div className="admin-new-article-options mt50px">
                ...
                <div className="admin-new-article-options_block">
                    <div className="admin-new-article-options_block_title"><span style={{marginRight: 20}}>Отложить публикацию</span><CustomCheckBox onClick={key => {
                ...
                <div className="admin-new-article-options_block">
                    ...
                    <input style={{fontSize: 14}} placeholder="Введите заголовок..." type="text" className="content admin-new-article_input-title" onChange={key => setArticleTitle(key.target.value)}/>
                    <div className="line" style={{minHeight: "1px", position: "absolute", bottom: 0, left: 0, width: "300px"}}/>
                </div>
            </div>
            <div className="mt50px">
                <input placeholder="Введите краткое описание..." type="text" className="content admin-new-article_input-title" onChange={key => setArticleDescription(key.target.value)}/>
                <div className="line"/>
            </div>
            <CustomEditor onChange={content => setArticleContent(content)}/>
            <button disabled={articleImage === null || articleTitle === "" || articleDescription === "" || articleContent?.blocks?.length === 0}
                    className="ui-button"
                    style={{width: "250px", position: "fixed", right: "5%", bottom: "5%", zIndex: 99}}
                    onClick={uploadArticle}>Опубликовать</button>
        </div>
    );
};
